...
from database import audit_collection, routes_collection
...
def get_all_routes(current_user: dict = Depends(get_current_user)):
    if current_user["role"] == "ADMIN":
        ...
        routes = list(
            routes_collection.find({"created_by": current_user["firebase_uid"]})
        )
...
@router.post("/routes")
def create_route(
    route: AddRoute, request: Request, current_user: dict = Depends(get_current_user)
):
    ...
    if document.get("start_latitude") is None:
        raise HTTPException(
            status_code=400,
            detail=f"Could not find address: {route_doc['starting_position']}",
        )
    for stop in document["stops"]:
        if stop.get("latitude") is None:
            raise HTTPException(
                status_code=400, detail=f"Could not find address: {stop['address']}"
            )
...
def get_route(id: str, current_user: dict = Depends(get_current_user)):
    ...
    if (
        ...
        and route["created_by"] != current_user["firebase_uid"]
    ):
...
def delete_route(
    id: str, request: Request, current_user: dict = Depends(get_current_user)
):
    route = routes_collection.find_one({"_id": ObjectId(id)})
    if not route:
        raise HTTPException(status_code=404, detail="Route not found")
    if (
        ...
        raise HTTPException(
            status_code=403, detail="You are not allowed to delete this route!"
        )
...
def get_optimized_route(
    ...
    if (
        ...
        raise HTTPException(
            status_code=403, detail="You are not allowed to optimize this route!"
        )
