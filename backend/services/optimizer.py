...
def nearest_neighbour(matrix:list, start: int=0)-> list:
    ...
    for i in range(n-1):
        ...
        for j in range(n):
            if(not visited[j]):
                if(matrix[currentStop][j] < nearest_dist):
                    nearest=j
                    nearest_dist=matrix[currentStop][j]
...
def optimize_route(route_doc: dict) -> dict:
    if not all(stop["latitude"] is not None and stop["longitude"] is not None for stop in route_doc["stops"]):
        return route_doc
